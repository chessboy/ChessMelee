...
open class PieceMovement {
    ...
    func canPieceMove(from fromLocation: BoardLocation,
        ...
        return isMovementPossible(from: fromLocation, to: toLocation, board: board)
    }
...
open class PieceMovementPawn: PieceMovement {
	...
	override func isMovementPossible(from fromLocation: BoardLocation,
		...
		if movingPiece.color == .black && toLocation.y == Constants.Chessboard.rowCount - 1 {
			return false
		}
		...
		if canMoveOneAhead {
			...
			} else if color == .black && fromLocation.y == Constants.Chessboard.rowCount - 2 {
				twoAheadStride = BoardStride(x: 0, y: -2)
			}
